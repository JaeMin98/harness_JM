...
- `Generator`: 합의된 sprint contract를 기준으로 구현한다.
- `Evaluator`: contract 기준으로 실제 동작을 검증하고 fail/pass를 판단한다.
- `Design Critic`: coherence, originality, craft, functionality 기준을 만든다.
...
```text
Generator와 Evaluator 관점으로 이번 sprint contract를 먼저 작성해줘. 이번 chunk의 scope, non-goals, done criteria, test criteria, security concerns를 문서 형태로 합의한 뒤에만 구현을 시작해줘.
```
...
```text
합의된 sprint contract 기준으로 이번 chunk만 구현해줘. 큰 범위로 확장하지 말고, 구현 후에는 self-check와 known gaps를 남겨줘.
```
...
```text
이제 Evaluator 관점으로 방금 구현한 chunk를 검토해줘. contract의 각 항목을 PASS or FAIL로 평가하고, FAIL이면 재현 경로와 수정 방향을 남겨줘. 아직 해결되지 않은 항목은 승인하지 마.
```
